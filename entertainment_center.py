...
data_string = "https://api.themoviedb.org/3/search/movie?api_key=0bf21ff60196bf22bce9136c1db1da7c&query="
movies = []
...
def create_movie_object(movie):
	...
	trailer = requests.get("http://api.themoviedb.org/3/movie/" + str(resp['id']) + "/videos?api_key=0bf21ff60196bf22bce9136c1db1da7c")
	youtube_link = "https://www.youtube.com/watch?v=" + trailer.json()['results'][0]['key']

	#add new movie object to list of movies
	movies.append(media.Movie(resp['original_title'], resp['overview'], poster_link, youtube_link))
